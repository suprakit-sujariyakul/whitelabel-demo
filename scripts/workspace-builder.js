...
// Main execution
const command = process.argv[2]; // 'build', 'clean', or 'clean-keep'
const client = process.env.CLIENT;
const env = process.env.ENV || 'dev';
const keepForDebugging = process.argv.includes('--keep') || command === 'clean-keep';
...
switch (command) {
	...
	case 'clean-keep':
		...
		break;
	default:
		console.error('Usage: node workspace-builder.js [build|clean|clean-keep]');
